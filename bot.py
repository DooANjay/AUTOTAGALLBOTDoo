...
async def menu_command(event):
    ...
    # Jika diakses dari grup log atau pm oleh entitas yang diizinkan
    commands_text = (
        "📋 **DAFTAR PERINTAH BOT TAGALL**\n\n"
        "**Perintah Umum (PM Bot):**\n"
        "> `/start` - Menampilkan pesan selamat datang dan menu interaktif\n\n"
        "**Perintah Admin/Owner (Log Group & PM Bot):**\n"
        "> `/menu` - Menampilkan daftar perintah ini\n"
        "> `/lpt` - Membuka panel manajemen data partner (PT)\n"
        "> `/cancel` - Membatalkan proses input/edit partner yang sedang berjalan\n"
        "> `/end` - Menghentikan paksa proses tagall yang sedang berlangsung"
    )
